...
from binance_client.client import Client
...
def request_tickers():
    tickers = []
    bclient = Client().get_exchange_info()
    for element in bclient['symbols']:
        tickers.append(element['symbol'])
    return tickers


def broadcast_signals(notifications_list):
    notifications_list.sort()
    ntb_str = '\nSignals: \n'
    for pair in notifications_list:
        for notification in pair:
...
def TA_task(a):
    ...
    klines = nbin_client.get_last_klines(ticker, interval, n_periods)
    for kline in klines:
    ...
    if threading and live:
        while True:
            with ThreadPoolExecutor(max_workers=16) as executor:
                ...
                broadcast_signals(notifications_list)
        ...
        while True:
            with tqdm(total=len(a)) as pbar:
                for tup in a:
                    notifications_list.append(TA_task(tup))
                    pbar.update(1)
            broadcast_signals(notifications_list)
            print('Sleeping {} sec now ... \n'.format(sleep))
            time.sleep(sleep)
